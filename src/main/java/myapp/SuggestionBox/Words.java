...
public class Words {
	...
	public static void delete_word(String content, boolean called) {
		...
		if (word.length != 2) {
			if (called) {
				Main.mainController.POPUP("Invalid deleting action!", false);
			}
		...
		try {
			...
			if (called) {
				Main.mainController.POPUP("Deleted success", true);
	...
	public static String[] getQuestionSet_meaning(char letter) {
		...
		for (String s : auto_complete_list) {
			...
			if (set.size() != old_size) {
				...
				try {
					String sql = "SELECT *FROM audio WHERE word_name= ? AND link IS NOT NULL";
					PreparedStatement preparedStatement = sqlite_connection.prepareStatement(sql);
